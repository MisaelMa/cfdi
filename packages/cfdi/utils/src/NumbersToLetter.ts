...
export class NumeroALetras {
  ...
  // Unidades()
  private Decenas(num: number): string | any {
  ...
  // Seccion()
  private Seccion(num: number, divisor: number, strSingular: string, strPlural: string) {
  ...
  NumeroALetras(num: number, curr: Currency) {
    const currency: Currency = curr || {} as Currency;
    const data = {
      numero: num,
