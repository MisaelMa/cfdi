...
export class A117 extends RPDF {
  ...
  protected addEmisorData(emisor: XmlEmisor, expedido: string) {
    this.docDefinition.content[0].columns[2].text[0].text =
      emisor._attributes!.Nombre + '\n';
    this.docDefinition.content[0].columns[2].text[1].text[1].text =
      emisor._attributes!.Rfc + '\n';
    const regimen = RegimenFiscalList.find(
      (f) => f.value.toString() === emisor._attributes!.RegimenFiscal
    );
    this.docDefinition.content[0].columns[2].text[2].text[1].text =
      emisor._attributes!.RegimenFiscal +
      ' - ' +
      regimen!.descripcion.toUpperCase() +
      '\n';
    this.docDefinition.content[0].columns[2].text[3].text[1].text = this.options
      .lugarExpedicion
      ? this.options.lugarExpedicion
      : expedido;
  }
    ...
    for (const detail of deatails) {
      ...
      if (detail['cfdi:ComplementoConcepto']) {
        if (detail['cfdi:ComplementoConcepto']['iedu:instEducativas']) {
          if (
            detail['cfdi:ComplementoConcepto']['iedu:instEducativas']
              ._attributes
          ) {
            const iedu =
              detail['cfdi:ComplementoConcepto']['iedu:instEducativas']
            ...
            descripcion.push({
              text: iedu!.nombreAlumno.toLocaleUpperCase() + '\n',
            });
            ...
            descripcion.push({
              text: iedu!.nivelEducativo.toLocaleUpperCase() + '\n',
            });
            descripcion.push({ text: 'CLAVE: ', bold: true });
            descripcion.push({
              text: iedu!.autRVOE.toLocaleUpperCase() + '\n',
            });
            descripcion.push({ text: 'RFC: ', bold: true });
            descripcion.push({
              text: iedu!.rfcPago.toLocaleUpperCase() + '\n',
            });
          }
        }
        if (
          detail['cfdi:ComplementoConcepto']['terceros:PorCuentadeTerceros']
        ) {
          // @ts-ignore
          const terceros =
            detail['cfdi:ComplementoConcepto']['terceros:PorCuentadeTerceros'];
        ...
        if (
          detail['cfdi:ComplementoConcepto']['ventavehiculos:VentaVehiculos']
        ) {
          // @ts-ignore
          const ventavehiculos =
            detail['cfdi:ComplementoConcepto']['ventavehiculos:VentaVehiculos'];
        }
      }
  ...
  protected addReceptor(receptor: XmlReceptor | undefined) {
    this.docDefinition.content[1].text[2] = {
      text: receptor
        ? receptor._attributes
          ? receptor._attributes.Nombre + '\n'
          : ''
        : '',
    };
    this.docDefinition.content[1].text[4] = {
      text: receptor
        ? receptor._attributes
          ? receptor._attributes.Rfc + '\n'
          : ''
        : '',
    };
    this.docDefinition.content[1].text[6] = {
      text: receptor
        ? receptor._attributes
          ? receptor._attributes.UsoCFDI + '\n'
          : ''
        : '',
    };
  }
  ...
  protected addCatidad(comprobante: CFDIComprobante) {
    ...
    this.docDefinition.content[3].table.body[0][1].text[3] = {
      text: '$' + comprobante.Descuento + '\n',
    };
    this.docDefinition.content[3].table.body[0][1].text[7] = {
      text: '$' + comprobante.Total + '\n',
    };
  }
  ...
  protected addNumberToLetter(total: number) {
    ...
    this.docDefinition.content[3].table.body[0][0].stack[1].text =
      nue.NumeroALetras(total, {
        plural: 'PESOS',
        singular: 'PESO',
        centPlural: 'CENTAVOS',
  ...
  protected fechaTimbrado(tfd: XmlTfd) {
    this.docDefinition.content[5].table.body[1][1].text =
      tfd._attributes.FechaTimbrado;
  }

  protected addCSDSat(tfd: XmlTfd) {
    this.docDefinition.content[5].table.body[3][1].text =
      tfd._attributes.NoCertificadoSAT;
  }

  protected folioFiscal(tfd: XmlTfd) {
    this.docDefinition.content[5].table.body[3][0].text =
      tfd._attributes.UUID.toUpperCase();
  }

  protected addSelloDgtEmisor(tfd: XmlTfd) {
    this.docDefinition.content[6].columns[1].table.body[1][0].text =
      tfd._attributes.SelloCFD;
  }

  protected addSelloDelSat(tfd: XmlTfd) {
    // @ts-ignore
    this.docDefinition.content[6].columns[1].table.body[3][0].text =
      tfd._attributes.SelloSAT;
  }
